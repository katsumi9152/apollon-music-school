...
非エンジニア向けの説明資料は別途 `docs/あぽろんレッスン日程_取扱説明書.pptx` を参照。
`README.md` は開発環境のセットアップ・実行手順が中心。本書はアプリの内部仕様(データフロー・モジュール契約・状態管理・制約)をまとめたもの。

最終更新: 2026-08-22 / 対象バージョン: `v20260823w`
...
- フッターの「紹介QR」ボタン(`#qr-btn`)は、常に DOM 上に存在する `#qr-modal`(`hidden` 属性で開閉)を表示/非表示するだけの単純なモーダル。QR 画像は `icons/qr-share.png`(アプリの公開 URL を指す静的画像、事前生成・埋め込み。クライアント側で QR を都度生成する処理は持たない)。閉じる操作は「✕ ボタン」「背景(バックドロップ)クリック」「Esc キー」の3経路。`.qr-modal` に `display: flex` を指定しているため、`[hidden]` を上書きしないよう `.qr-modal[hidden] { display: none; }` を明示している(CSS の `hidden` 属性オーバーライドの既知の落とし穴)。
...
6. アプリの公開 URL が変わった場合(独自ドメイン化等)は、`icons/qr-share.png` を新しい URL で再生成すること。この画像は URL を埋め込んだ静的ファイルであり、自動追従はしない。
