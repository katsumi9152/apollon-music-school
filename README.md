...
`index.html` の `<script>` / `<link>` にはすべて `?v=<カウンター>-<gitハッシュ>` を付けている(GitHub Pages のキャッシュを回避するため)。`styles.css` や `src/*.js` を1文字でも変えたら、コミット前に必ず実行すること。

```powershell
pwsh -File bump-version.ps1
```

`VERSION`(コミットする永続カウンターファイル)を1つ進め、`index.html` 内のバージョン文字列を全箇所まとめて書き換える。書き手をこのスクリプト1つに絞ることで、「7箇所のうち1箇所だけ上げ忘れる」事故を防いでいる。画面下の `.app-version` にも同じ文字列を表示しているので、実機で最新が反映されているか確認できる。

埋め込まれる git ハッシュは実行時点の `HEAD`(＝直前のコミット)を指す。このアプリはビルド成果物を配布するものではなく `index.html` 自体がそのまま配信されるため、ハッシュが「このコミット自身」を指せない(自己参照になる)ことは実用上問題にならない。
...
| `manifest.webmanifest` / `icons/` | 「ホーム画面に追加」用のアプリ名・テーマ色・アイコン |
| `src/app.js` | DOM 層(店舗選択・データ取得・描画・イベント処理) |
| `VERSION` / `bump-version.ps1` | キャッシュ対策バージョン文字列の永続カウンターと更新スクリプト |
